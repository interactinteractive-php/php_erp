...
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b)
{
    ...
    if (format == 'xml') {
        ...
        ExportDialog.saveLocalFile(editorUi, mxUtils.getXml(graph.getSvg(bg, s, b)), name, format, w, h);
 ...
 * key=value, where value should be URL encoded.
 */
ExportDialog.saveLocalFile = function(editorUi, data, filename, format, w, h)
{
    if (data.length < MAX_REQUEST_SIZE) {
        
        editorUi.hideDialog();
        
        var req = new mxXmlRequest(EXPORT_URL, 'xml='+encodeURIComponent(data)+'&filename='+encodeURIComponent(filename)+'&format='+format+'&w='+w+'&h='+h);
        req.simulate(document, '_blank');
