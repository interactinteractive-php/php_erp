...
StyleFormatPanel.prototype.init = function()
{
	...
	var graph = editor.graph;
	var ss = this.format.getSelectionState();
        var shapeName = ss.style.shape;
	
	if (ss.containsImage && ss.vertices.length == 1 && shapeName == 'image' &&
	...
	if (!ss.containsImage || shapeName == 'image')
	{
	...
	this.container.appendChild(this.addStyleOps(opsPanel));
        ...
        var ignoreShapes = [
            'connector', 'swimlane', 'mxgraph.flowchart.annotation_2', 'flexArrow'
        ];
        
	if (!ignoreShapes.includes(shapeName) && graph.getSelectionCount() == 1) {
            
            this.container.appendChild(this.addMetaOps(this.createPanel()));
        }
};
 ...
 * Adds the label menu items to the given menu and parent.
 */
StyleFormatPanel.prototype.addMetaOps = function(div)
{
    var btn = null;

    btn = mxUtils.button('Мета', mxUtils.bind(this, function(evt) {
        this.editorUi.actions.get('editStyle').funct();
    }));

    btn.setAttribute('title', 'Мета');
    btn.style.width = '202px';
    btn.style.marginBottom = '2px';
