...
(function(factory){
  ...
  function buildHierarchy($appendTo, nodeData, level, opts, callback){
    ...
    if(Object.keys(nodeData).length > 1){ // if nodeData has nested structure
      ...
      $.when(createNode(nodeData, level, opts))
              .done(function($nodeDiv){
                $table.append($nodeDiv.wrap('<tr><td' + (hasChildren ? ' colspan="' +
                        $childNodes.length * 2 + '"' : '') + '></td></tr>').closest('tr'));
                if(callback){
                  callback();
                }
              })
    ...
    if(hasChildren){
      ...
      $table.append('<tr class="lines' + isHidden + '"><td colspan="' +
              $childNodes.length * 2 + '"><div class="down"></div></td></tr>');
