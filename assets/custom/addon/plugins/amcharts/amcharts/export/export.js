    ...
    var _this={
      ...
      drawing: {
        ...
        handler: {
          ...
          add: function(options){
            ...
            var method=cfg.url.indexOf(".svg") != -1 ? fabric.loadSVGFromURL :
                    fabric.Image.fromURL;

            method(cfg.url, function(objects, options){
              var group=options !== undefined ? fabric.util.groupSVGElements(objects, options) :
                      objects;
              var ratio=false;

              // RESCALE ONLY IF IT EXCEEDS THE CANVAS
              if(group.height > _this.setup.fabric.height || group.width >
                      _this.setup.fabric.width){
      ...
      defaults: {
        ...
        pdfMake: {
          ...
          images: {},
          content: [globSavedFrom, window.location.href, {
              image: "reference",
              fit: [523.28, 769.89]
            }]
        },
      ...
      i18l: function(key, language){
        var lang=language ? langugage : _this.setup.chart.language ? _this.setup.chart.language :
                "en";
        var catalog=AmCharts.translations[ _this.name ][ lang ] ||
                AmCharts.translations[ _this.name ][ "en" ];
      ...
      download: function(data, type, filename){
        ...
        if(window.saveAs && _this.setup.hasBlob){
          var blob=_this.toBlob({
            ...
            filename='chart_' + Metronic.getFullDateFromDate() + '.' + ext;
...
//					} ] );
      ...
      isTainted: function(source){
        var origin=String(window.location.origin || window.location.protocol + "//" +
      ...
      gatherElements: function(group, cfg, images){
        ...
        for(i1=0; i1 < group.children.length; i1++){
          ...
          if(childNode.tagName == "clipPath"){
            var bbox={};
            var transform=fabric.parseTransformAttribute(_this.gatherAttribute(childNode,
                    "transform"));
      ...
      capture: function(options, callback){
        ...
        fabric.ElementsParser.prototype.resolveGradient=function(obj, property){
          ...
          var gradientId=instanceFillValue.slice(instanceFillValue.indexOf("#") + 1,
                  instanceFillValue.length - 1);
          if(fabric.gradientDefs[ this.svgUid ][ gradientId ]){
            obj.set(property, fabric.Gradient.fromElement(
                    fabric.gradientDefs[ this.svgUid ][ gradientId ], obj));
          }
        };
        ...
        if(_this.config.legend && _this.setup.chart.legend && _this.setup.chart.legend.position ==
                "outside"){
          var group={
            ...
            legend: {
              type: ["top", "left"].indexOf(_this.config.legend.position) != -1 ? "unshift" :
                      "push",
              position: _this.config.legend.position,
              width: _this.config.legend.width ? _this.config.legend.width :
                      _this.setup.chart.legend.container.width,
              height: _this.config.legend.height ? _this.config.legend.height :
          ...
          if(["left", "right"].indexOf(group.legend.position) != -1){
            ...
            offset.height=group.legend.height > offset.height ? group.legend.height :
                    offset.height;
        ...
        _this.setup.wrapper.setAttribute("class", _this.setup.chart.classNamePrefix +
                "-export-canvas");
        ...
        if(_this.setup.chart.type == "stock"){
          ...
          if(_this.setup.chart.periodSelector && ["top", "bottom"].indexOf(
                  _this.setup.chart.periodSelector.position) != -1){
            offset.height-=_this.setup.chart.periodSelector.offsetHeight +
                    _this.setup.chart.panelsSettings.panelSpacing;
            padding[ _this.setup.chart.periodSelector.position ]+=
                    _this.setup.chart.periodSelector.offsetHeight +
                    _this.setup.chart.panelsSettings.panelSpacing;
          }
          if(_this.setup.chart.dataSetSelector && ["top", "bottom"].indexOf(
                  _this.setup.chart.dataSetSelector.position) != -1){
            offset.height-=_this.setup.chart.dataSetSelector.offsetHeight;
            padding[ _this.setup.chart.dataSetSelector.position ]+=
                    _this.setup.chart.dataSetSelector.offsetHeight;
          }
        ...
        _this.setup.fabric.on("mouse:move", function(e){
          ...
          if(_this.drawing.buffer.isPressed && !_this.drawing.buffer.line){
            if(!_this.drawing.buffer.isSelected && _this.drawing.mode != "pencil" && (p.xD > 5 ||
                    p.xD > 5)){
        ...
        for(i1=0; i1 < groups.length; i1++){
          var group=groups[ i1 ];
          var isLegend=_this.gatherClassName(group.parent, _this.setup.chart.classNamePrefix +
                  "-legend-div", 1);
          var isPanel=_this.gatherClassName(group.parent, _this.setup.chart.classNamePrefix +
                  "-stock-panel-div");
          var isScrollbar=_this.gatherClassName(group.parent, _this.setup.chart.classNamePrefix +
                  "-scrollbar-chart-div");
          ...
          if(_this.setup.chart.type == "stock" && _this.setup.chart.legendSettings.position){
            ...
            } else if(_this.setup.chart.legend && ["left", "right"].indexOf(
                    _this.setup.chart.legend.position) != -1){
              group.offset.y=_this.pxToNumber(group.parent.style.top);
              group.offset.x=_this.pxToNumber(group.parent.style.left);
            }
          }
          ...
          fabric.parseSVGDocument(group.svg, (function(group){
            return function(objects, options){
              ...
              for(i1=0; i1 < g.paths.length; i1++){
                ...
                if(g.paths[ i1 ]){
                  ...
                  if(g.paths[ i1 ].TSPANWORKAROUND){
                    var parsedAttributes=fabric.parseAttributes(g.paths[ i1 ].svg,
              ...
              if(group.svg.parentNode && group.svg.parentNode.getElementsByTagName){
                var balloons=group.svg.parentNode.getElementsByClassName(
                        _this.setup.chart.classNamePrefix + "-balloon-div");
                for(i1=0; i1 < balloons.length; i1++){
                  if(cfg.balloonFunction instanceof Function){
                    ...
                    var fabric_label=new fabric.Text(elm_parent.innerText ||
                            elm_parent.textContent || elm_parent.innerHTML, {
                              selectable: false,
              ...
              if(group.svg.nextSibling && group.svg.nextSibling.tagName == "A"){
                ...
                var style_parent=fabric.parseStyleAttribute(elm_parent);
                var fabric_label=new fabric.Text(elm_parent.innerText || elm_parent.textContent ||
                        elm_parent.innerHTML, {
                          selectable: false,
                          top: style_parent.top + group.offset.y,
                          left: style_parent.left + group.offset.x,
      ...
      toByteArray: function(options, callback){
        ...
        function b64ToByteArray(b64){
          ...
          for(i=0, j=0; i < l; i+=4, j+=3){
            tmp=(decode(b64.charAt(i)) << 18) | (decode(b64.charAt(i + 1)) << 12) | (decode(
                    b64.charAt(i + 2)) << 6) | decode(b64.charAt(i + 3))
          ...
          if(placeHolders === 2){
            ...
            tmp=(decode(b64.charAt(i)) << 10) | (decode(b64.charAt(i + 1)) << 4) | (decode(
                    b64.charAt(i + 2)) >> 2)
            push((tmp >> 8) & 0xFF)
            push(tmp & 0xFF)
          }
      ...
      getChartData: function(options){
        ...
        var uid, i1, i2, i3;
        var lookupFields=["valueField", "openField", "closeField", "highField", "lowField",
          "xField", "yField"];
        ...
        if(cfg.data.length){
          ...
          for(i1=0; i1 < cfg.data.length; i1++){
            ...
            for(i2=0; i2 < cfg.dataFields.length; i2++){
              ...
              var title=(cfg.columnNames && cfg.columnNames[ uniqueField ]) ||
                      cfg.titles[ uniqueField ] || uniqueField;
      ...
      createMenu: function(list, container){
        ...
        function buildList(list, container){
          ...
          for(i1=0; i1 < list.length; i1++){
            ...
            // FILTER; TOGGLE FLAG
            if(["CSV", "JSON", "XLSX"].indexOf(item.format) != -1 && ["map", "gauge"].indexOf(
                    _this.setup.chart.type) != -1){
              continue;

              // BLOB EXCEPTION
            } else if(!_this.setup.hasBlob && item.format != "UNDEFINED"){
              if(item.mimeType && item.mimeType.split("/")[ 0 ] != "image" && item.mimeType !=
                      "text/plain"){
                continue;
              }
            }
                  ...
                  return function(){
                    if(item.capture || item.action == "print" || item.format == "PRINT"){
                      this.capture(item, function(){
                        ...
                        this[ "to" + item.format ](item, function(data){
                          if(item.action == "download"){
                            this.download(data, item.mimeType, [item.fileName, item.extension
                            ].join("."));
                          }
                        });
                      })
            ...
            a.addEventListener("click", (function(callback, item){
              return function(e){
                ...
                // DELAYED
                if((item.action == "draw" || item.format == "PRINT" || (item.format !=
                        "UNDEFINED" && item.capture)) && !_this.drawing.enabled){
            ...
            if(item.icon){
              img.setAttribute("src", (!item.ignore && item.icon.slice(0, 10).indexOf("//") == -1 ?
                      chart.pathToImages : "") + item.icon);
              a.appendChild(img);
            }
            if(item.label){
              a.appendChild(span);
        ...
        _this.setup.menu.setAttribute("class", _this.setup.chart.classNamePrefix +
                "-export-menu " + _this.setup.chart.classNamePrefix + "-export-menu-" +
                _this.config.position + " amExportButton");
      ...
      delay: function(options, callback){
        ...
        var menu=_this.createMenu([{
            label: _this.i18l("capturing.delayed.menu.label").replace("{{duration}}",
                    AmCharts.toFixed(cfg.delay, cfg.precision)),
        ...
        t1=setInterval(function(){
          ...
          if(diff <= 0){
            ...
            label.innerHTML=_this.i18l("capturing.delayed.menu.label").replace("{{duration}}",
                    AmCharts.toFixed(diff, 2));
          }
        }, 10);
      ...
      clear: function(){
        ...
        if(listenersToRemove){
          for(var i=0; i < listenersToRemove.length; i++){
            ...
            listenerToRemove.node.removeEventListener(listenerToRemove.event,
                    listenerToRemove.method)
          }
        }
        _this.listenersToRemove=[];
      },
      ...
      construct: function(){
        ...
        // MERGE SETTINGS
        _this.config.drawing=_this.deepMerge(_this.defaults.fabric.drawing, _this.config.drawing ||
                {}, true);
                if(_this.config.border){
                  _this.config.border=_this.deepMerge(_this.defaults.fabric.border,
                          _this.config.border || {}, true);
                }
    ...
    if(config){
      ...
      _this.config=_this.migrateSetup(_this.setup.chart.amExport ||
              _this.setup.chart.exportConfig);
