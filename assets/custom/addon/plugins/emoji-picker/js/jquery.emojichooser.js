...
;(function($) {
  ...
  var categories = [
    ...
    { name: 'symbol', label: 'Symbols', icon: '&#x269b;' },
    /*{ name: 'flag', label: 'Flags', icon: '&#x1f3f3;' }*/
  ];
  ...
  $.extend(EmojiChooser.prototype, {
    ...
    loadPrefs: function() {
        ...
        $.each(emojiChooserEmojis, function(i, emoji) {
    ...
    createChooser: function() {
      ...
      // Show template
      this.$chooser = $( getChooserHTML(this) )
      ...
      // Chooser height
      this.$chooser.find('.emoji-sections')
        .height(parseInt(this.settings.height) - 32); // 40 is height of the tabs
      ...
      */
      ...
      if (this.settings.mode != 'chat') {
            ...
            if (this.settings.position == 'top') {
              elOffset.top = elOffset.top - parseInt(this.settings.height) - 10;
              elOffset.left = elOffset.left - 1;

              if (this.settings.footer) {
                  elOffset.top = elOffset.top - 42;
              }
            } else {
        ...
        // Step 3
        var diffOffset = {
          bottom: this.$el.innerHeight() + 18,
          left: (elOffset.left - parentOffset.top)
        };
        
         this.$chooser.css({
          bottom: diffOffset.bottom,
          left: diffOffset.left
        });
     }
    ...
    emojiMouseover: function(e) {
      ...
      $shortcode.find('.info').show().html('<div class="emoji emoji-' + emojiShortcode + '" data-shortcode="' + emojiShortcode + '" title="' + emoji.name + '">'+getHtmlEntities(emoji.unicode)+'</div><em>' + emojiShortcode + '</em>');
    },
    ...
    emojiCategoryClicked: function(e) {
	  ...
	  var that = this;
      // Update tab
      that.$chooser.find('nav .tab').removeClass('active');
      ...
      var $section = that.$chooser.find('section.' + section);
      ...
      that.$chooser.find('.emoji-sections').off('scroll'); // Disable scroll event until animation finishes

      that.$chooser.find('.emoji-sections').animate({
        scrollTop: scrollDistance
      }, 250, function() {
        that.$chooser.find('.emoji-sections').on('scroll', $.proxy(that.emojiScroll, that) ); // Enable scroll event
      });
    },
    ...
    emojiScroll: function(e) {
      var sections = this.$chooser.find('section');
    ...
    searchCharEntered: function(e) {
      ...
      var searchEmojis = $(e.target).parents('.emoji-sections').find('section.search');
      var searchEmojiWrap = searchEmojis.find('.wrap');
      var sections = $(e.target).parents('.emoji-sections').find('section');
      ...
      if (searchTerm.length > 0) {
        ...
        $.each(emojiChooserEmojis, function(i, emoji) {
          ...
          if ( shortcode.indexOf(searchTerm) > -1 ) {
            results.push('<em><span class="emoji emoji-' + shortcode + '" data-shortcode="' + shortcode + '" title="' + emoji.name + '">'+getHtmlEntities(emoji.unicode)+'</span></em>');
          }
        });
  ...
  function getChooserHTML(opt) {
    ...
    nodes.push('</nav>');
    nodes.push('<div class="emoji-sections">');

    // Search
    nodes.push('<section class="search">');
    nodes.push('<input type="search" placeholder="Search..." class="form-control">');
    ...
    if (localStorageSupport) {
      ...
      for (var i = recentlyUsedEmojis.length-1; i > -1 ; i--) {
        ...
        if (emoji != null) {
            nodes.push('<em><span class="emoji emoji-' + recentlyUsedEmojis[i] + '" data-shortcode="' + recentlyUsedEmojis[i] + '" title="' + emoji.name + '">'+getHtmlEntities(emoji.unicode)+'</span></em>');
        }
      }
    ...
    for (var i = 0; i < categories_length; i++) {
      ...
      for (var j = 0; j < category_length; j++) {
        ...
        nodes.push('<em><span class="emoji emoji-' + emoji.shortcode + '" data-shortcode="' + emoji.shortcode + '" title="' + emoji.name + '">'+getHtmlEntities(emoji.unicode)+'</span></em>');
      }
      nodes.push('</div></section>');
    }
  ...
  function updateRecentlyUsed(emoji) {
    ...
    var currentScrollTop = $('.emoji-sections:visible').scrollTop();
	...
	if (false) {
		$('.emoji-sections:visible').animate({
