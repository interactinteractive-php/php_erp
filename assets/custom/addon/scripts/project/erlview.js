var IS_LOAD_ERLVIEW_SCRIPT = true;
var $cvlUniqHtmlContainerView = '';
var jqTreeData = [];
var $globalTree;

function electronRegisterLegalViewInit(elem, processMetaDataId, dataViewId, selectedRow, paramData, $type) {
    ...
    $tree.on('tree.select', function (event) {
        if (event.node) {
            ...
            if (typeof $type !== 'undefined' && ($type == '2' || $type == '4' || $type == '6' || $type == '7')) {
                ...
                $treeUrl = 'mddoc/electronViewTreeDataV2';
                $companyKeyId = ($type == '7' || $type == '6') ? event.node.folderid : event.node.id;
            }
