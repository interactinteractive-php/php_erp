...
class RequestTest extends TestCase
{
    ...
    public function testCreateWithRequestUri()
    {
        ...
        $request = Request::create('http://test.com:8084/foo');
        $request->server->set('REQUEST_URI', 'http://test.com:8084/foo');
        $this->assertEquals('http://test.com:8084/foo', $request->getUri());
        $this->assertEquals('/foo', $request->getPathInfo());
        $this->assertEquals('test.com', $request->getHost());
        $this->assertEquals('test.com:8084', $request->getHttpHost());
        $this->assertEquals(8084, $request->getPort());
        $this->assertFalse($request->isSecure());
    ...
    public function getRequestUriData()
    {
        ...
        yield ['https://test.com:8084/foo', '/foo', $message];
    ...
    public function testCreateCheckPrecedence()
    {
        ...
        $request = Request::create('http://thomas:pokemon@example.net:8084/?foo=bar', 'GET', [], [], [], [
            'HTTP_HOST' => 'example.com',
            'HTTPS' => 'on',
            'SERVER_PORT' => 443,
        ]);
    ...
    public function testGetUri()
    {
        ...
        // http://host:8084/index.php/path/info?query=string

        $server['HTTP_HOST'] = 'host:8084';
        ...
        $this->assertEquals('http://host:8084/index.php/path/info?query=string', $request->getUri(), '->getUri() with non default port');
        ...
        // http://host:8084/path/info?query=string
        $server = [];
        $server['HTTP_HOST'] = 'host:8084';
        $server['SERVER_NAME'] = 'servername';
        $server['SERVER_PORT'] = '8084';
        ...
        $request->initialize([], [], [], [], [], $server);
        $this->assertEquals('http://host:8084/path/info?query=string', $request->getUri(), '->getUri() with rewrite');
        ...
        $this->assertEquals(
            'http://host:8084/ba%20se/index_dev.php/foo%20bar/in+fo?query=string',
            $request->getUri()
        );
        ...
        $request->initialize([], [], [], [], [], $server);
        $this->assertEquals('http://host:8084/ba%20se/index_dev.php/foo%20bar/in+fo?query=string', $request->getUri());
    ...
    public function testGetUriForPath()
    {
        ...
        $this->assertEquals('http://host:8084/index.php/some/path', $request->getUriForPath('/some/path'), '->getUriForPath() with non default port');
        ...
        $request->initialize([], [], [], [], [], $server);
        $this->assertEquals('http://host:8084/some/path', $request->getUriForPath('/some/path'), '->getUri() with rewrite');
    ...
    public function testGetHost()
    {
        ...
        $request->initialize([], [], [], [], [], ['HTTP_HOST' => 'www.example.com:8084']);
    ...
    public function testTrustedProxiesXForwardedFor()
    {
        ...
        $request->headers->set('X_FORWARDED_HOST', 'foo.example.com:1234, real.example.com:8084');
        $request->headers->set('X_FORWARDED_PROTO', 'https');
    ...
    public function testLegacyTrustedProxies()
    {
        ...
        $request->server->set('REMOTE_ADDR', '3.3.3.3');
        $request->headers->set('X_FORWARDED_FOR', '1.1.1.1, 2.2.2.2');
        $request->headers->set('X_FORWARDED_HOST', 'foo.example.com, real.example.com:8084');
        $request->headers->set('X_FORWARDED_PROTO', 'https');
        $request->headers->set('X_FORWARDED_PORT', 443);
    ...
    public function testTrustedProxiesForwarded()
    {
        $request = Request::create('http://example.com/');
        $request->server->set('REMOTE_ADDR', '3.3.3.3');
        $request->headers->set('FORWARDED', 'for=1.1.1.1, host=foo.example.com:8084, proto=https, for=2.2.2.2, host=real.example.com:8084');
    ...
    public function nonstandardRequestsData()
    {
        return [
            ...
            ['hello/app.php/x',  '', '/x', 'http://host:8084/hello/app.php/x', '/hello', '/hello/app.php'],
            ['/hello/app.php/x', '', '/x', 'http://host:8084/hello/app.php/x', '/hello', '/hello/app.php'],

            ['',      'a=b', '/', 'http://host:8084/?a=b'],
            ['?a=b',  'a=b', '/', 'http://host:8084/?a=b'],
            ['/?a=b', 'a=b', '/', 'http://host:8084/?a=b'],

            ['x',      'a=b', '/x', 'http://host:8084/x?a=b'],
            ['x?a=b',  'a=b', '/x', 'http://host:8084/x?a=b'],
            ['/x?a=b', 'a=b', '/x', 'http://host:8084/x?a=b'],

            ['hello/x',  '', '/x', 'http://host:8084/hello/x', '/hello'],
            ['/hello/x', '', '/x', 'http://host:8084/hello/x', '/hello'],

            ['hello/app.php/x',      'a=b', '/x', 'http://host:8084/hello/app.php/x?a=b', '/hello', '/hello/app.php'],
            ['hello/app.php/x?a=b',  'a=b', '/x', 'http://host:8084/hello/app.php/x?a=b', '/hello', '/hello/app.php'],
            ['/hello/app.php/x?a=b', 'a=b', '/x', 'http://host:8084/hello/app.php/x?a=b', '/hello', '/hello/app.php'],
        ];
    }
    ...
    public function testNonstandardRequests($requestUri, $queryString, $expectedPathInfo, $expectedUri, $expectedBasePath = '', $expectedBaseUrl = null)
    {
        ...
        $this->assertEquals(8084, $request->getPort());
        $this->assertEquals('host:8084', $request->getHttpHost());
    ...
    public function testTrustedHost()
    {
        ...
        $request = Request::create('/');
        $request->server->set('REMOTE_ADDR', '1.1.1.1');
        $request->headers->set('Forwarded', 'host=localhost:8084');
        $request->headers->set('X-Forwarded-Host', 'localhost:8084');

        $this->assertSame('localhost:8084', $request->getHttpHost());
        $this->assertSame(8084, $request->getPort());
    ...
    public function testTrustedPort()
    {
        ...
        $request->server->set('REMOTE_ADDR', '1.1.1.1');
        $request->headers->set('Forwarded', 'host=localhost:8084');
        $request->headers->set('X-Forwarded-Port', 8084);
